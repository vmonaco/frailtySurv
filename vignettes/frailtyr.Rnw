...
Background, other packages available, motivation

\begin{table}[h]
\protect\caption{R functions for fitting survival models. P=parametric, NP=nonparametric,
PPL=partial penalized likelihood, MML=maximum marginal likelihood, DLs=downloads.}
\centering{}
...
Model description, overview of estimation procedure.

Two methods of estimation, use of LT to solve integrals, performance

fitfrail arguments, return value

\section{Simulation}

Generating data, genfrail arguments. Example generating data and fitting model
...
Using real-world data, various frailty distributions.
...
TODO: For each frailty distribution: density, Laplace transform, method to generate random samples
