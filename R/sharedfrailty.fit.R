...
sharedfrailty.fit <- function(x, y, cluster, beta_init, theta_init, frailty_distr, control, rownames) {
  ...
  # The order of failure (or time index) for the data, where k==1 is t==0
  k <- 1 + rank(time)
  ...
  # N_[[i]][j, k] indicates whether individual j in family i failed at time <= tau_k
  ...
  # N_dot[[i]][k] is the number of individuals in family i that failed up to time tau_k
  N_dot <- sum_by_family(N_)
  
  # Y_[[i]][j,k] indicates whether individual j in family i failed at time >= tau_k
  ...
  # Create psi_i for each family. Depends on theta, N_i.(t) and H_i.(t)
  psi_ <- sapply(cluster_names, function (i) {
    function(theta, N_dot_ik, H_dot_ik) {
      ...
      # For gamma, (N_dot[[i]][k] + 1/theta)/(H_dot[[i]][k] + 1/theta)
  ...
  estimate_lambda_hat <- function(beta, theta) {
    ...
    H_ <- sapply(cluster_names, function(i) {
      matrix(0, family_sizes[[i]], length(time_steps))
    }, simplify = FALSE, USE.NAMES = TRUE)
    ...
    for (k in 2:length(time_steps)) {
      ...
      denom <- sum(sapply(cluster_names, function(i) {
        psi_[[i]](theta, N_dot[[i]][k-1], H_dot[[i]][k-1]) * 
          sum(sapply(1:family_sizes[[i]], function(j) {
